// api/ujicoba.js
// Satu file untuk semua endpoint ujicoba — hemat Serverless Function slot
// Route via query param: ?action=get-soal | get-hasil | simpan-hasil | save-soal | verify-admin | delete-hasil

export default async function handler(req, res) {
  const action = req.query.action;

  if (!action) {
    return res.status(400).json({ error: 'Parameter ?action diperlukan.' });
  }
  ...
  if (data.soal && data.soal[0]?.pertanyaan !== undefined) {
    ...
    return {
      ...
      soal: data.soal.map(s => ({
        ...
        opsi: typeof s.opsi==='object'&&!Array.isArray(s.opsi)?[s.opsi.A,s.opsi.B,s.opsi.C,s.opsi.D]:s.opsi,
        jawaban: typeof s.jawaban==='string'?(li[s.jawaban.toUpperCase()]??0):s.jawaban
      }))
    };
  }
  return { nama: data.nama||namaDefault[tesKey]||tesKey, durasi: data.durasi||durasiDefault[tesKey]||3600, soal: data.soal||[] };
}
...
async function handleSimpanHasil(req, res) {
  ...
  const { GITHUB_PAT, GITHUB_OWNER, GITHUB_REPO } = env;
  const body = req.body;
  if (!body?.tesKey || body.skor === undefined) return res.status(400).json({ error: 'Body harus mengandung tesKey dan skor.' });
  ...
  try {
    ...
    list.push({
      tesKey: body.tesKey, tesNama: body.tesNama || body.tesKey.toUpperCase(),
      pesertaNama: (body.pesertaNama||'Anonim').substring(0,60),
      skor: Number(body.skor)||0, totalSoal: Number(body.totalSoal)||0,
      waktuDipakai: Number(body.waktuDipakai)||0,
      timestamp: body.timestamp || new Date().toISOString()
    });
    if (list.length > 1000) list = list.slice(list.length - 1000);
    const put = await ghPut(url, GITHUB_PAT, `[ujicoba] Hasil tes: ${body.tesNama} - ${body.pesertaNama}`, list, existing.sha);
    if (!put.ok) return res.status(put.status).json({ error: `Gagal simpan ke GitHub: ${put.status}` });
...
async function handleSaveSoal(req, res) {
  ...
  const { GITHUB_PAT, GITHUB_OWNER, GITHUB_REPO } = env;
  const body = req.body;
  if (!body?.tesKey || !Array.isArray(body.soal)) return res.status(400).json({ error: 'Body harus mengandung tesKey dan array soal.' });
  ...
  try {
    ...
    const put = await ghPut(url, GITHUB_PAT, `[admin] Update soal: ${body.tesKey} (${body.soal.length} soal)`, payload, existing.sha||null);
    if (!put.ok) return res.status(put.status).json({ error: `Gagal simpan ke GitHub: ${put.status}` });
...
async function handleDeleteHasil(req, res) {
  if (req.method !== 'POST' && req.method !== 'DELETE') return res.status(405).json({ error: 'Gunakan POST.' });
  const env = checkEnv(res); if (!env) return;
  if (!checkAdmin(req, res)) return;
  const { GITHUB_PAT, GITHUB_OWNER, GITHUB_REPO } = env;
  const body = req.body;
  ...
  try {
    ...
    if (mode === 'reset') {
      ...
      if (isNaN(i) || i < 0 || i >= list.length) return res.status(400).json({ error: `Index ${i} tidak valid.` });
      list.splice(i, 1); deleted = 1;
    } else if (mode === 'filter') {
      const before = list.length;
      list = list.filter(e => !(e.pesertaNama === body.pesertaNama && (!body.tesKey || e.tesKey === body.tesKey)));
      deleted = before - list.length;
    } else {
      return res.status(400).json({ error: `Mode tidak dikenal: ${mode}` });
    }

    const put = await ghPut(url, GITHUB_PAT, `[admin] Hapus ${deleted} entri scoreboard`, list, existing.sha);
    if (!put.ok) return res.status(put.status).json({ error: `Gagal simpan ke GitHub: ${put.status}` });
